...
int processBitmap(unsigned int xMap, unsigned int yMap, int *x1, int *y1,
		int *x2, int *y2);
int main() {

	int x1, x2, y1, y2;

	int xmap = 0b011100000111000111000;
	int ymap = 0b1111000;

	printf("xmap = %d\n",xmap);
	printf("ymap = %d\n",ymap);

	processBitmap(xmap,ymap,&x1,&y1,&x2,&y2);
...
int processBitmap(unsigned int xMap, unsigned int yMap, int *x1, int *y1,
	...
	*y1 = ( modelData.y_max * ( 2 * yLow.start_bit + yLow.num_bits - 1 ) )
	...
	printf( "ps2: Process bitmap, fingers=%d, x=%d, y=%d\n", fingers,
			abs( *x1 - *x2 ) / fingers_x, abs( *y1 - *y2 ) / fingers_y );
	return fingers;
}
