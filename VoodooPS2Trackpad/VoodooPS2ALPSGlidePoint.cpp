...
{ { 0x63, 0x02, 0x64 }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8, 0 }, { { 0x63, 0x03,
		0xc8 }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8, ALPS_PASS | ALPS_DUALPOINT }, /* Dell Latitude D800 */
{ { 0x73, 0x00, 0x0a }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8,
ALPS_DUALPOINT }, /* ThinkPad R61 8918-5QG */
{ { 0x73, 0x02, 0x0a }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8, 0 }, { { 0x73, 0x02,
		0x14 }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8,
ALPS_FW_BK_2 }, /* Ahtec Laptop */
{ { 0x20, 0x02, 0x0e }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8,
ALPS_PASS | ALPS_DUALPOINT }, /* XXX */
{ { 0x22, 0x02, 0x0a }, 0x00, ALPS_PROTO_V2, 0xf8, 0xf8,
ALPS_PASS | ALPS_DUALPOINT }, { { 0x22, 0x02, 0x14 }, 0x00, ALPS_PROTO_V2, 0xff,
		0xff, ALPS_PASS | ALPS_DUALPOINT }, /* Dell Latitude D600 */
...
ApplePS2ALPSGlidePoint *ApplePS2ALPSGlidePoint::probe(IOService *provider,
		SInt32 *score) {
...
bool ApplePS2ALPSGlidePoint::deviceSpecificInit() {
	...
	init_fail: IOLog(
			"%s: Device initialization failed. Touchpad probably won't work\n",
			getName() );
	resetMouse();
	return false;
}
...
bool ApplePS2ALPSGlidePoint::resetMouse() {
	...
	// Verify the result
	if (request.commands[1].inOrOut != kSC_Reset
			&& request.commands[2].inOrOut != kSC_ID) {
		DEBUG_LOG(
				"Failed to reset mouse, return values did not match. [0x%02x, 0x%02x]\n",
...
void ApplePS2ALPSGlidePoint::packetReady() {
	...
	while (_ringBuffer.count() >= modelData.pktsize) {
		...
		if (( packet[0] & modelData.mask0 ) == modelData.byte0) {
			DEBUG_LOG(
					"ps2: Got pointer event with packet = { %02x, %02x, %02x, %02x, %02x, %02x }\n",
					packet[0], packet[1], packet[2], packet[3], packet[4],
					packet[5] );
...
void ApplePS2ALPSGlidePoint::processPacketV1V2(UInt8 *packet) {
	...
	if (modelData.flags & ALPS_WHEEL) {
		...
		int scrollAmount = ( ( packet[2] << 1 ) & 0x08 )
				- ( ( packet[0] >> 4 ) & 0x07 );
		if (scrollAmount) {
...
void ApplePS2ALPSGlidePoint::processTrackstickPacketV3(UInt8 *packet) {
	...
	if (packet[1] == 0x7f && packet[2] == 0x7f && packet[3] == 0x7f) {
		DEBUG_LOG(
				"ps2: ignoring trackstick packet that indicates end of stream\n" );
		return;
	}
...
void ApplePS2ALPSGlidePoint::processTouchpadPacketV3(UInt8 *packet) {
	...
	if (!modelData.multi_packet && ( f.first_mp )) {
		DEBUG_LOG(
				"ps2: trackPad: detected multi-packet first packet, waiting to handle\n" );
...
void ApplePS2ALPSGlidePoint::decodeButtonsV3(struct alps_fields *fields,
		UInt8 *packet) {
...
void ApplePS2ALPSGlidePoint::decodePinnacle(struct alps_fields *f, UInt8 *p) {
	...
	f->fingers = ( p[5] & 0x3 ) + 1;
	f->x_map = ( ( p[4] & 0x7e ) << 8 ) | ( ( p[1] & 0x7f ) << 2 )
			| ( ( p[0] & 0x30 ) >> 4 );
	f->y_map = ( ( p[3] & 0x70 ) << 4 ) | ( ( p[2] & 0x7f ) << 1 )
			| ( p[4] & 0x01 );
...
void ApplePS2ALPSGlidePoint::decodePacketV6(struct alps_fields *f, UInt8 *p) {
	DEBUG_LOG( "Alps decode : " );
...
void ApplePS2ALPSGlidePoint::decodeDolphin(struct alps_fields *f, UInt8 *p) {
	...
	f->fingers = ( ( p[0] & 0x6 ) >> 1 | ( p[0] & 0x10 ) >> 2 );
	f->x_map = ( ( p[2] & 0x60 ) >> 5 ) | ( ( p[4] & 0x7f ) << 2 )
			| ( ( p[5] & 0x7f ) << 9 ) | ( ( p[3] & 0x07 ) << 16 )
			| ( ( p[3] & 0x70 ) << 15 ) | ( ( p[0] & 0x01 ) << 22 );
	f->y_map = ( p[1] & 0x7f ) | ( ( p[2] & 0x1f ) << 7 );
...
void ApplePS2ALPSGlidePoint::dispatchEventsWithInfo(int xraw, int yraw, int z,
		int fingers, UInt32 buttonsraw) {
	uint64_t now_abs;
	clock_get_uptime( &now_abs );
	uint64_t now_ns;
	...
	DEBUG_LOG(
			"%s::dispatchEventsWithInfo: x=%d, y=%d, z=%d, fingers=%d, buttons=%d\n",
	...
	if (!clickpadtype || fingers == 3) {
		buttons = middleButton( buttons, now_abs,
				fingers == 3 ? fromPassthru : fromTrackpad );
	...
	if (z < z_finger && isTouchMode()) {
		...
		if (now_ns - touchtime < maxtaptime && clicking) {
			...
			if (( touchmode == MODE_DRAG || touchmode == MODE_DRAGLOCK )
					&& ( draglock || draglocktemp
							|| ( dragTimer && dragexitdelay ) )) {
	...
	if (isTouchMode() && isFingerTouch( z )) {
		...
		if (!wasdouble && !wastriple
				&& ( dx > tapthreshx || dy > tapthreshy )) {
			touchtime = 0;
	...
	switch (touchmode) {
		case MODE_DRAG:
		case MODE_DRAGLOCK:
			if (MODE_DRAGLOCK == touchmode
					|| ( !immediateclick || now_ns - touchtime > maxdbltaptime )) {
				buttons |= 0x1;
			}
			// fall through
		case MODE_MOVE:
		...
		case MODE_MTOUCH:
			...
			switch (fingers) {
				case 1:
					...
					DEBUG_LOG(
							"Transition from multitouch to single touch and move\n" );
					calculateMovement( x, y, z, fingers, dx, dy );
					break;
				case 2: // two finger
					if (last_fingers != fingers) {
				...
				case 3: // three finger
					...
					ymoved += dy;
					DEBUG_LOG(
							"xmoved=%d, ymoved=%d, inSwipeUp=%d, inSwipeRight=%d, inSwipeLeft=%d, inSwipeDown=%d\n",
							xmoved, ymoved, inSwipeUp, inSwipeRight,
							inSwipeLeft, inSwipeDown );
					...
					if (ymoved < -swipedy && !inSwipeUp && !inSwipe4Up) {
						...
						ymoved = 0;
						DEBUG_LOG( "swipe up\n" );
						_device->dispatchKeyboardMessage( kPS2M_swipeUp,
								&now_abs );
						break;
					}
					if (ymoved > swipedy && !inSwipeDown && !inSwipe4Down) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipeDown,
								&now_abs );
						break;
					}
					if (xmoved > swipedx && !inSwipeRight && !inSwipe4Right) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipeRight,
								&now_abs );
						break;
					}
					if (xmoved < -swipedx && !inSwipeLeft && !inSwipe4Left) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipeLeft,
								&now_abs );
						break;
					}
					break;
				...
				case 4: // four fingers
					...
					DEBUG_LOG(
							"xmoved=%d, ymoved=%d, inSwipeUp=%d, inSwipeRight=%d, inSwipeLeft=%d, inSwipeDown=%d\n",
							xmoved, ymoved, inSwipe4Up, inSwipe4Right,
							inSwipe4Left, inSwipe4Down );
					// Reset relative movement so we don't actually report that
					// there was regular movement
					dx = 0;
					dy = 0;
					...
					if (ymoved < -swipedy && !inSwipe4Up) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipe4Up,
								&now_abs );
						break;
					}
					if (ymoved > swipedy && !inSwipe4Down) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipe4Down,
								&now_abs );
						break;
					}
					if (xmoved > swipedx && !inSwipe4Right) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipe4Right,
								&now_abs );
						break;
					}
					if (xmoved < -swipedx && !inSwipe4Left) {
						...
						_device->dispatchKeyboardMessage( kPS2M_swipe4Left,
								&now_abs );
						break;
					}
			}
		...
		case MODE_VSCROLL:
			if (!vsticky && ( x < redge || fingers > 1 || z > zlimit )) {
				DEBUG_LOG(
						"Switch back to notouch. redge=%d, vsticky=%d, zlimit=%d\n",
		...
		case MODE_HSCROLL:
			if (!hsticky && ( y < bedge || fingers > 1 || z > zlimit )) {
				DEBUG_LOG(
						"Switch back to notouch. bedge=%d, hsticky=%d, zlimit=%d\n",
		...
		case MODE_CSCROLL:
			...
			dispatchScrollWheelEventX( ( dx + scrollrest ) / cscrolldivisor, 0,
					0, now_abs );
		...
		case MODE_PREDRAG:
			if (!immediateclick
					&& ( !palm_wt || now_ns - keytime >= maxaftertyping )) {
				buttons |= 0x1;
	...
	if (scroll && cscrolldivisor) {
		if (touchmode == MODE_NOTOUCH && z > z_finger && y > tedge
				&& ( ctrigger == 1 || ctrigger == 9 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && y > tedge && x > redge
				&& ( ctrigger == 2 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && x > redge
				&& ( ctrigger == 3 || ctrigger == 9 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && x > redge && y < bedge
				&& ( ctrigger == 4 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && y < bedge
				&& ( ctrigger == 5 || ctrigger == 9 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && y < bedge && x < ledge
				&& ( ctrigger == 6 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && x < ledge
				&& ( ctrigger == 7 || ctrigger == 9 ))
			touchmode = MODE_CSCROLL;
		if (touchmode == MODE_NOTOUCH && z > z_finger && x < ledge && y > tedge
				&& ( ctrigger == 8 ))
			touchmode = MODE_CSCROLL;

		DEBUG_LOG( "new touchmode=%d\n", touchmode );
	}
	if (( MODE_NOTOUCH == touchmode
			|| ( MODE_HSCROLL == touchmode && y >= bedge ) ) && z > z_finger
			&& x > redge && vscrolldivisor && vscroll) {
		DEBUG_LOG(
				"switch to vscroll touchmode redge=%d, bedge=%d, vscrolldivisor=%d, scroll=%d\n",
				redge, bedge, vscrolldivisor, scroll );
		touchmode = MODE_VSCROLL;
		scrollrest = 0;
	}
	if (( MODE_NOTOUCH == touchmode
			|| ( MODE_VSCROLL == touchmode && x <= redge ) ) && z > z_finger
			&& y > bedge && hscrolldivisor && hscroll && vscroll) {
...
void ApplePS2ALPSGlidePoint::calculateMovement(int x, int y, int z, int fingers,
		int &dx, int &dy) {
...
int ApplePS2ALPSGlidePoint::processBitmap(unsigned int xMap, unsigned int yMap,
	...
	*y1 = ( modelData.y_max * ( 2 * yLow.start_bit + yLow.num_bits - 1 ) )
	...
	DEBUG_LOG(
			"ps2: Process bitmap, fingers=%d, x1=%d, x2=%d, y1=%d, y2=%d, area = %d\n",
...
void ApplePS2ALPSGlidePoint::dispatchRelativePointerEventWithPacket(
		UInt8 *packet, UInt32 packetSize) {
	//
	...
	if (( packet[0] & 0x1 ))
		buttons |= 0x1;  // left button   (bit 0 in packet)
	if (( packet[0] & 0x2 ))
		buttons |= 0x2;  // right button  (bit 1 in packet)
	if (( packet[0] & 0x4 ))
		buttons |= 0x4;  // middle button (bit 2 in packet)
...
bool ApplePS2ALPSGlidePoint::enterCommandMode() {
	...
	if (( status.bytes[0] != 0x88
			|| ( status.bytes[1] != 0x07 && status.bytes[1] != 0x08 ) )
			&& status.bytes[0] != 0x73) {
...
bool ApplePS2ALPSGlidePoint::hwInitV3() {
	...
	if (regVal == 0
			&& setupTrackstickV3( ALPS_REG_BASE_PINNACLE )
					== kIOReturnIOError) {
		IOLog( "Failed to setup trackstick\n" );
		goto error;
	}

	if (!( enterCommandMode() && absoluteModeV3() )) {
...
int ApplePS2ALPSGlidePoint::commandModeReadReg(int addr) {
	...
	DEBUG_LOG(
			"ApplePS2ALPSGlidePoint read reg result: { 0x%02x, 0x%02x, 0x%02x }\n",
			status.bytes[0], status.bytes[1], status.bytes[2] );
	...
	if (addr != ( ( status.bytes[0] << 8 ) | status.bytes[1] )) {
		DEBUG_LOG(
				"ApplePS2ALPSGlidePoint ERROR: read wrong registry value, expected: %x\n",
				addr );
		return -1;
	}
...
bool ApplePS2ALPSGlidePoint::commandModeSendNibble(int nibble) {
	...
	if (nibble > 0xf) {
		IOLog(
				"%s::commandModeSendNibble ERROR: nibble value is greater than 0xf, command may fail\n",
				getName() );
	}
...
//    }
	...
	if (send > 0) {
		request.commands[cmdCount].command =
				kPS2C_SendMouseCommandAndCompareAck;
		request.commands[cmdCount++].inOrOut =
				modelData.nibble_commands[nibble].data;
	}
...
bool ApplePS2ALPSGlidePoint::passthroughModeV3(int regBase, bool enable) {
	...
	if (!enterCommandMode()) {
		IOLog(
				"ERROR: Failed to enter command mode while enabling passthrough mode\n" );
		return false;
	}
	...
	error: if (!exitCommandMode()) {
		IOLog(
				"ERROR: failed to exit command mode while enabling passthrough mode v3\n" );
		return false;
	}
...
IOReturn ApplePS2ALPSGlidePoint::setupTrackstickV3(int regBase) {
	...
	error: if (!passthroughModeV3( regBase, false )) {
		ret = kIOReturnIOError;
	}
...
bool ApplePS2ALPSGlidePoint::repeatCmd(SInt32 init_command, SInt32 init_arg,
		SInt32 repeated_command, ALPSStatus_t *report) {
	...
	DEBUG_LOG( "%02x report: [0x%02x 0x%02x 0x%02x]\n", repeated_command,
			report->bytes[0], report->bytes[1], report->bytes[2] );
...
IOReturn ApplePS2ALPSGlidePoint::identify() {
	 ...
	 * pressed.
	 */
	if (!repeatCmd( kDP_SetMouseResolution, NULL, kDP_SetMouseScaling1To1,
			&e6 )) {
	...
	if (( e6.bytes[0] & 0xf8 ) != 0 || e6.bytes[1] != 0
			|| ( e6.bytes[2] != 10 && e6.bytes[2] != 100 )) {
	 ...
	 * most ALPS touchpads.
	 */
	if (!( repeatCmd( kDP_SetMouseResolution, NULL, kDP_SetMouseScaling2To1,
			&e7 )
			&& repeatCmd( kDP_SetMouseResolution, NULL, kDP_MouseResetWrap,
					&ec ) && exitCommandMode() )) {
	...
	IOLog(
			"%s::identify: Found ALPS Device with ID E7=0x%02x 0x%02x 0x%02x, EC=0x%02x 0x%02x 0x%02x\n",
			getName(), e7.bytes[0], e7.bytes[1], e7.bytes[2], ec.bytes[0],
	...
	if (matchTable( &e7, &ec )) {
		...
		return 0;
	} else if (ec.bytes[0] == 0x88 && ec.bytes[1] == 0x07 && ec.bytes[2] >= 0x90
			&& ec.bytes[2] <= 0x9d) {
		modelData.proto_version = ALPS_PROTO_V3;
		setDefaults();
	...
	IOLog(
			"Unknown ALPS touchpad, does not match any known identifiers: E7=0x%02x 0x%02x 0x%02x, EC=0x%02x 0x%02x 0x%02x\n",
			e7.bytes[0], e7.bytes[1], e7.bytes[2], ec.bytes[0], ec.bytes[1],
			ec.bytes[2] );
